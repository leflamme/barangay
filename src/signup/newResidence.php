...
try {  
    ...
    // C. Residence Status
    $sql_residence_status = "INSERT INTO `residence_status` (`residence_id`, `status`,`archive`,`pwd`,`pwd_info`,`single_parent`,`senior`, `date_added`) VALUES (?,?,?,?,?,?,?,?)";
    $stmt_status = $con->prepare($sql_residence_status);
    $stmt_status->bind_param('ssssssss',$number,$add_status,$archive,$add_pwd,$add_pwd_check,$add_single_parent,$senior,$date_added);
    $stmt_status->execute();
    $stmt_status->close();
