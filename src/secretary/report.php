...
// --- 1. INITIALIZE VARIABLES (Prevents "Undefined Variable" Errors) ---
$total_households = 0;
$total_residents = 0;
$total_seniors = 0;
$total_pwd = 0;
$household_rows = ''; // To store the table HTML
$first_name_user = '';
$last_name_user = '';
$user_image = '';
$barangay = '';
...
try {
    ...
    if($brgy_info = $result_brgy->fetch_assoc()){
    ...
    // =================================================================================
    // LOGIC 1: HOUSEHOLD MONITORING DASHBOARD
    // =================================================================================
    ...
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $search = $_GET['search'];
        // Search logic: household number, address, or head name
    ...
    $base_sql = "SELECT 
                    ...
                    h.date_created,
                    h.household_head_id,
                    -- Subqueries for accurate counting
    ...
    // =================================================================================
    // LOGIC 2: RESIDENT MASTERLIST FILTERING
    // =================================================================================
    ...
    $query_report = $con->query($resident_query);
    
    if($query_report && $query_report->num_rows > 0){
        ...
        $.ajax({
            ...
            success: function(response) {
                if(response.success === true) {
                    ...
                    $('#membersList').html('<p class="text-danger text-center">Error loading data: ' + response.message + '</p>');
                }
            },
            error: function(xhr, status, error) {
                console.error("AJAX Error: ", status, error);
                $('#membersList').html('<p class="text-danger text-center">Connection failed. Check console.</p>');
            }
        });
    });
});
